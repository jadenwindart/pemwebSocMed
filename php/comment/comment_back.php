<?php
    class Comment {
        ...
        public function __construct($id, $username, $name, $comment, $comment_time) {
            $this->id = $id;
    ...
    if(!empty($visiting_username)) {
        ...
        //Get Comment
        $col = 'c.comment_id, c.username, u.first_name, u.last_name, c.comment, TIMESTAMPDIFF(minute, c.comment_date, NOW()) AS time_diff';
        ...
        while($row_comment = $result->fetch_assoc()) {
            ...
            $comment[] = new Comment($row_comment['comment_id'], $row_comment['username'], $temp_name, $row_comment['comment'], $row_comment['time_diff']);
        }
        // var_dump($comment);
